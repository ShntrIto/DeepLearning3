...
def conv2d_simple(x, W, b=None, stride=1, pad=0):
    ...
    Weight = W
    N, C, H, W = x.shape # 入力のバッチ数とそのサイズ
    OC, C, KH, KW = Weight.shape # カーネルの個数とそのサイズ
    SH, SW = pair(stride) # stride をペアにする（int だけの入力でも同じ数でペアを作る）
    PH, PW = pair(pad) # pad をペアにする（int だけの入力でも同じ数でペアを作る）
    OH = get_conv_outsize(H, KH, SH, PH) # 出力のサイズを計算
    OW = get_conv_outsize(W, KW, SW, PW) # 出力のサイズを計算
    
    # REVIEW: カーネルのサイズだけは Weight から決める?
    col = im2col(x, (KH, KW), stride, pad, to_matrix=True)
    Weight = Weight.reshape(OC, -1).transpose()
    t = linear(col, Weight, b)
    y = t.reshape(N, OH, OW, OC).transpose(0, 3, 1, 2) # (N,C,H,W) に変換
    return y
...
def pooling_simple(x, kernel_size, stride=1, pad=0):
    ...
    col = im2col(x, kernel_size, stride, pad, to_matrix=True) # いつものように展開
    col = col.reshape(-1, KH * KW) # 次に，チャンネル別にしたカーネルサイズのデータを取り出す
    y = col.max(axis=1) # 行ごとに最大値を取り出すだけで max pooling ができる
